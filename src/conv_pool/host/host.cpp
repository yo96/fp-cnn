...
int main(int argc, char* argv[]) {
  ...
  const int ofmap_out_size = ofmap_nblk * ofmap_wid * ofmap_ht * ofmap_dep/BASE_PER_OBUS;
  ...
  const int num_ddr_trans = ofmap_ht * ofmap_ddr_wid * ofmap_nblk;
  ...
  std::cout << "\nReLU:" << std::endl
            << "  fil_size in outbus  : " << fil_out_size << std::endl
            << "  ofmap_size in outbus: " << ofmap_out_size << std::endl;
  ...
  krnl_relu.setArg(0, fil_out_size  ); // fil_size
  krnl_relu.setArg(1, ofmap_out_size); // o_size

  krnl_pool.setArg(0, buf_out       ); // o_fmap
  krnl_pool.setArg(1, pool_size     ); // pool_size
  krnl_pool.setArg(2, ofmap_wid     ); // o_wid
  krnl_pool.setArg(3, ofmap_out_size); // o_size
