...
int main(int argc, char* argv[]) {
  ...
  assert(fmap_size==_conv1_in_size && out_size==_conv1_out_size); 

  std::vector<base,aligned_allocator<base>> src_fmap(conv1_in[0], conv1_in[0]+_conv1_in_size);
  std::vector<base,aligned_allocator<base>> src_wts (conv1_w,  conv1_w+_conv1_w_size);
  std::vector<base,aligned_allocator<base>> src_out (out_size,  0);
  std::vector<base,aligned_allocator<base>> ref     (conv1_out[0], conv1_out[0]+_conv1_out_size);
