...
class CnnAccelerator {
  ...
  public: 
    ...
    void run_inference( const cl::Buffer& buf_fmap, cl::Buffer& buf_out ) {
      ...
      //cl::Buffer buf_fmap(_w.context, CL_MEM_USE_HOST_PTR  | CL_MEM_READ_ONLY,
                          //fmap_Bsize, src_fmap.data(), NULL);
      //cl::Buffer buf_out (_w.context, CL_MEM_USE_HOST_PTR  | CL_MEM_WRITE_ONLY,
      ...
      //for (int i=0;i<out_size;i++){
        //if (src_out[i] != ref[i]){
        ////if (_w.src_out[i] != _w.ref[i]){
          //std::cout << "cpu[" << i << "] = " << ref[i] 
                    //<< ", device[" << i << "] = " << src_out[i] << std::endl;
          ////std::cout << "cpu[" << i << "] = " << _w.ref[i] 
                    ////<< ", device[" << i << "] = " << _w.src_out[i] << std::endl;
          //match = false;
        //}
      //}
  ...
  private:
    ...
    void exec_layer( const layer_arg_t& arg, const cl::Buffer& buf_fmap, 
      ...
      //q->enqueueMigrateMemObjects({buf_fmap, buf_wts, buf_out}, 0); [> 0 means from host<]
      //q->finish(); 
